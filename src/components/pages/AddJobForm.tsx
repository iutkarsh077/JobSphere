...
import { NewJobCreated } from "../../../actions/NewJobCreated";
...
const AddJobForm = () => {
  ...
  const form = useForm<AddJobSchema>({
    resolver: AddJobResolver,
    defaultValues: {
      jobTitle: "",
      slug: "",
      location: "",
  ...
  const handleJobUpload = async (data: AddJobSchema) => {
    data.image = imageUrl;
    // console.log(data);
    if(!imageUrl){
        toast({
    ...
    try {
        setIsLoading(true);
        const res = await NewJobCreated(data);
        form.reset();
    } catch (error) {
        // console.log(error);
        toast({
