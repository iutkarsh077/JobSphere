      ...
      const getJobs = async () =>{
          try {
              ...
              if (!jobData1.status) {
                ...
                const myjob = jobData1.getAllLatestJobs?.slice(22, 24).reverse();
                const job1 = jobData1.getAllLatestJobs?.slice(0, 4).reverse();
                const mergedJobs = myjob?.concat(job1!);
                  setJobData(mergedJobs);
          } catch (error) {
              toast({
