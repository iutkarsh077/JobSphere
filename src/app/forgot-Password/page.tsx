...
import { OtpForForgotPassword } from "../../../actions/otpForForgotPassword";
...
const ForgotPassword = () => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      setIsLoading(true);
      // console.log(myEmail)
      const res = await OtpForForgotPassword(myEmail);
      if (!res.status) {
  ...
  const handleVerifyEmail = async (data: any) => {
    try {
      ...
      toast({
        title: res.message,
        description: "Email Verified Successfully, Now you can Change Password!"
      });
  ...
  return (
    <>
    {
        !NewPassword ? (<div className="h-screen w-screen flex justify-center items-center bg-gray-900 bg-center bg-cover" style={{ backgroundImage: "url('/images/jobBG2.jpeg')" }}>
          ...
          </div>): (<ConfirmPassword email={myEmail}/>)
    }
    </>
  );
};
